...
firaz = firearray(1:end,4);
firedoy = firearray(1:end,2)
fireazindex = find(fireaz>180)
fireazindex = firepksnr(fireazindex)
length(fireazindex)
mean(fireazindex)
%%
...
[contaz] = find(contaz>180);
contazindex = contpksnr(contaz);
length(contpksnr)
mean(contpksnr)
%%
%Normal plots

csmooth= smooth(contazindex(1:53));
...
title('Moving Average, P346 Azimuth Specific (degrees>180 & degrees<360)')
